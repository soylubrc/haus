        ...
        <?php else: ?>
            <div class="tours-grid">
                <?php foreach ($duraklar as $durak): ?>
                    <?php 
                    $turTamamlanmis = isset($durak['teslim_edildi']) && $durak['teslim_edildi'] == 1;
                    $turBekleniyor = isset($durak['bekleniyor']) && $durak['bekleniyor'] == 1;
                    
                    $statusClass = $turTamamlanmis ? 'status-delivered' : 
                                  ($turBekleniyor ? 'status-waiting' : 'status-error');
                                  
                    $statusBadgeClass = $turTamamlanmis ? 'status-badge-delivered' : 
                                       ($turBekleniyor ? 'status-badge-waiting' : 'status-badge-error');
                                       
                    $statusText = $turTamamlanmis ? 'Teslim Edildi' : 
                                 ($turBekleniyor ? 'Bekleniyor' : 'Hata');
                                 
                    $statusIcon = $turTamamlanmis ? 'fa-check-circle' : 
                                 ($turBekleniyor ? 'fa-clock' : 'fa-exclamation-triangle');
                    ?>
